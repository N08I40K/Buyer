package ru.n08i40k.buyer.commands.subcommands.locale;
...
public class LocaleSetCommand extends SubCommand {
    public LocaleSetCommand(@NotNull SubCommand parentCommand) {
        super(parentCommand);
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            localeRequestBuilder.get("select-locale", String.join(", ", locales))
                    .getSingle().sendMessage(sender);
            return false;
        }

        if (!locales.contains(args[0])) {
            localeRequestBuilder.get("incorrect-locale", String.join(", ", locales))
                    .getSingle().sendMessage(sender);
            return false;
        }
        ...
        localeRequestBuilder.get("successful")
                .getSingle().sendMessage(sender);
        return true;
    }
